...
export default function EntrenamientoCompleto({ data }: Props) {
  ...
  const descargarReporte = async (formato: FormatoReporte) => {
    ...
    try {
      ...
      setErrorDescarga('No se pudo descargar el reporte.')
    } finally {
      setDescargando(null)
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="text-base font-bold text-indigo-800 flex items-center gap-2">
              <FlaskConical className="w-5 h-5" /> Experimentación completa — 5 modelos
            </h2>
            <p className="text-xs text-indigo-600 mt-0.5">
              {dataset.registros} días ({dataset.fecha_inicio} → {dataset.fecha_fin}) ·{' '}
              {data.n_datos_entrenamiento} entrenamiento / {data.n_datos_prueba} prueba
            </p>
            <p className="text-xs text-gray-400 flex items-center gap-1 mt-1">
              <Clock className="w-3 h-3" /> Tiempo total: {duracion_total_segundos}s
            </p>
          </div>
          <div className="flex items-center gap-2 bg-white border border-indigo-300 rounded-lg px-3 py-2 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">MAE = {mae_ganador} · {criterio_seleccion}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-2 flex items-center gap-2 text-xs">
          ...
          {modelo_guardado.guardado ? (
            <span className="text-green-700">
              Modelo ganador guardado ({modelo_guardado.tipo_modelo}) — se puede predecir sin reentrenar.
            </span>
          ) : (
            <span className="text-gray-400">
              {modelo_guardado.motivo || 'El modelo ganador no se guardó en esta ejecución.'}
            </span>
          )}
        </div>
        ...
        {ejecucion_id && (
          <div className="mt-3 pt-3 border-t border-indigo-100">
            <p className="text-xs text-gray-500 mb-1.5">Descargar reporte de esta ejecución (#{ejecucion_id}):</p>
      ...
      <div className="card">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">
          ...
          <span className="text-xs font-normal text-gray-400 ml-1">(valores menores → mayor precisión)</span>
        </h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600" title="U2 menor que 1 supera al pronostico ingenuo">U Theil</th>
                  ...
                  <tr key={key} className={esGanador ? 'bg-indigo-50 font-semibold' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      {m.error ? (
                        ...
                        <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full font-medium">Ganador</span>
                      ) : (
                        <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Evaluado</span>
                      )}
                    </td>
                  </tr>
                )
      ...
      <div className="card">
        <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
          <Sliders className="w-4 h-4 text-indigo-500" /> Optimización de hiperparámetros
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                <th className="text-left px-3 py-2 text-xs font-semibold text-gray-600">Modelo</th>
                <th className="text-left px-3 py-2 text-xs font-semibold text-gray-600">Mejor configuración</th>
                <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600">Mejor MAE</th>
                <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600">Combinaciones</th>
                <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600">Tiempo</th>
                <th className="text-right px-3 py-2 text-xs font-semibold text-gray-600">Semilla</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {NOMBRES_5_MODELOS.map((key) => {
                ...
                return (
                  <tr key={key} className="hover:bg-gray-50">
                    ...
                    {hp.aplicable ? (
                      ...
                      <td colSpan={5} className="px-3 py-2.5 text-xs text-gray-400 italic">{hp.motivo || 'No aplicable'}</td>
                    )}
                  </tr>
                )
              })}
      ...
      <div className="card">
        <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
          <Layers className="w-4 h-4 text-indigo-500" /> Validación cruzada temporal (walk-forward)
        </h3>
              ...
              <details key={key} className="border border-gray-200 rounded-lg overflow-hidden" open={key === modelo_ganador}>
                <summary className="cursor-pointer px-3 py-2 bg-gray-50 text-sm font-medium text-gray-700 flex items-center justify-between">
                  ...
                  <span className="text-xs text-gray-400">
                    {cv.n_folds_ejecutados}/{cv.n_folds_solicitados} folds · MAE prom. {fmt(cv.promedio.mae)} ± {fmt(cv.desviacion_estandar.mae)}
                  </span>
                </summary>
                <div className="p-3">
                  ...
                  {cv.folds.length > 0 ? (
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b">
                            <th className="text-left px-2 py-1 font-semibold text-gray-500">Fold</th>
                            <th className="text-left px-2 py-1 font-semibold text-gray-500">Train</th>
                            <th className="text-left px-2 py-1 font-semibold text-gray-500">Validación</th>
                            ...
                            <th className="text-right px-2 py-1 font-semibold text-gray-500" title="U2 menor que 1 supera al pronostico ingenuo">U Theil</th>
                            <th className="text-right px-2 py-1 font-semibold text-gray-500">Tiempo</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                          {cv.folds.map((f) => (
                    ...
                    <p className="text-xs text-gray-400">Sin folds ejecutados.</p>
                  )}
                </div>
              </details>
            )
      ...
      <div className="card">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">Pruebas estadísticas (alpha = 0.05)</h3>
        ...
        <div className={`rounded-lg border p-3 mb-3 ${pruebas_estadisticas.friedman.significativo ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'}`}>
          <p className="text-xs font-semibold text-gray-700">
            Friedman — comparación simultánea de los 5 modelos
          </p>
          ...
          {pruebas_estadisticas.friedman.aplicable && (
            <div className="flex gap-4 mt-2 text-xs">
              <span>Estadístico: <strong className="font-mono">{fmt(pruebas_estadisticas.friedman.estadistico)}</strong></span>
              <span>p-valor: <strong className="font-mono">{fmt(pruebas_estadisticas.friedman.p_valor)}</strong></span>
            </div>
          )}
        </div>
        ...
        {pruebas_estadisticas.wilcoxon.length > 0 && (
          <div className="mb-3">
            <p className="text-xs font-semibold text-gray-700 mb-2">
              Wilcoxon — {modelo_ganador_legible} vs. cada competidor (corrección Holm-Bonferroni)
            </p>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b">
                    <th className="text-left px-2 py-1 font-semibold text-gray-500">Competidor</th>
                    <th className="text-right px-2 py-1 font-semibold text-gray-500">p-valor</th>
                    <th className="text-right px-2 py-1 font-semibold text-gray-500">p-valor ajustado</th>
                    <th className="text-center px-2 py-1 font-semibold text-gray-500">Significativo</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
        ...
        {pruebas_estadisticas.diebold_mariano.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-gray-700 mb-2">Diebold-Mariano — ganador vs. cada competidor</p>
            <div className="space-y-2">
      ...
      {predicciones_futuras.length > 0 && (
        <div className="card">
          <h3 className="text-base font-semibold text-gray-700 mb-1">Predicciones futuras — {modelo_ganador_legible}</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={predicciones_futuras}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="demanda_estimada" stroke="#6366f1" strokeWidth={2.5} name="Demanda estimada" dot={{ r: 4 }} />
              <Line type="monotone" dataKey="recomendacion" stroke="#f59e0b" strokeWidth={2} strokeDasharray="6 3" name="Producción recomendada" dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
